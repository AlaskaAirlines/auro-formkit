...
| input | 71 | 352 | +281 | 88.70% | 99.59% | +10.89% |
| layoutElement | 0 | 50 | +50 | N/A | 100% | +100% |
| menu¹ | 55 | 304 | +249 | 80.47% | 96.97% | +16.50% |
...
| form | New `web-test-runner.config.mjs` with `timeout: 10000` — form fixtures register every form element (input, datepicker, counter, combobox, select, radio, checkbox, menu, button), exceeding the default 2s mocha timeout under turbo parallelism |
...
| counter | Missing `ariaDescribedByElements` on spinbutton — A11Y tests hung because `deep.equal(null, [element])` serialized DOM infinitely. Fixed by adding `onDescriptionSlotChange` handler in `auro-counter.js` |
| datepicker | Three test expectations corrected: `updateCentralDate()` normalizes to 1st of month (not the exact date), `setMonth()` returns a timestamp (not a Date object) |
