...
Three test assertions were corrected to match actual component behavior:

1. **`calendarFocusDate`** — Expected `03/23/2023` but `updateCentralDate()` normalizes the date to the 1st of the month. Corrected to `03/01/2023`.
2. **`minDate`** — Same normalization issue. Expected `04/09/2023`, corrected to `04/01/2023`.
3. **`handleMonthChange`** — `setMonth()` returns a timestamp (number), not a `Date` object. Wrapped the result in `new Date()` before the `instanceOf(Date)` assertion.
