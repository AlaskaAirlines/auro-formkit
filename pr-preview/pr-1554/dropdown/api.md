...
| `noFocusRestoreOnClose`   |                           | `Symbol()`                                       | false            | When true, the dropdown skips its generic focus restoration on close.<br />Set by consumers (e.g. combobox) that manage their own focus routing<br />via setClearBtnFocus / setInputFocus / keyboard strategy.<br />Separate from noHideOnThisFocusLoss (which controls auto-close behavior). |
