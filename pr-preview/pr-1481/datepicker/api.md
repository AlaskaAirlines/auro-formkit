...
| `monthNames`                      | `monthNames`                      |           | `array`             |                | Long-form month names. Auto-derived from the resolved locale unless the<br />consumer assigns a 12-element array override.<br /><br />F1: Stays a getter (not a reactive property + eager populate) because<br />Cally renders its own month header from its `locale` attribute — the<br />datepicker shell does not read `monthNames` during render. The array is<br />only computed when consumer code reads it, then cached per locale. |
