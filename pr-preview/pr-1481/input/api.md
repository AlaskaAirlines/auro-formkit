...
| `layout`                          |                                   | `string`                                         |             |                                                  |
| `locale`                          | `locale`                          | `string`                                         |             | BCP47 locale tag such as `en-US`, `de-DE`, or `ja-JP`.<br />When set, drives the date input mask pattern for `type="date"`.<br />May also be inherited from a `data-locale` attribute on an ancestor element via `DomHandler.getLocale`. |
