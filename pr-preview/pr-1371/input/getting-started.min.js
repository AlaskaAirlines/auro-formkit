...
class BaseInput extends AuroElement {
  ...
  updated(changedProperties) {
    ...
    } else if (changedProperties.has('locale') && this.type === 'date') {
      // When the locale changes, and if the format was not explicitly set or was previously the default for the old locale,
      // update to the new locale's default format.

      if (!this.format || this.format.toLowerCase() === getDateFormatFromLocale(changedProperties.get('locale'))) {
        const nextLocaleDefaultFormat = getDateFormatFromLocale(this.locale);
