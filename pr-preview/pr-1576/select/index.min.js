...
var formkitVersion$1 = '202608031914';
...
function resolveSelectedOptions(items, valueArray, selectedKeys) {
  ...
  // and its value is part of the requested value set.
  if (Array.isArray(selectedKeys)) {
    selectedKeys.forEach((key) => {
      const keyed = items.find((item) => item._optionKey === key);
      if (keyed && isSelectableByValue(keyed) && valueArray.includes(keyed.value) && !resolved.includes(keyed)) {
        resolved.push(keyed);
      }
    });
  }

  // Fall back to value matching for any values not yet resolved by key. Match by
  // count, not just presence: start from a copy of `valueArray` and remove one
  // entry per already-resolved option, so a value that appears twice but was only
  // resolved once by key still leaves a second occurrence to match here.
  const unresolvedValues = [...valueArray];
  resolved.forEach((option) => {
    const idx = unresolvedValues.indexOf(option.value);
    if (idx >= 0) {
      unresolvedValues.splice(idx, 1);
    }
  });
  unresolvedValues.forEach((val) => {
  ...
  // keys/values were selected.
  resolved.sort((optionA, optionB) => items.indexOf(optionA) - items.indexOf(optionB));
...
class AuroMenu extends AuroElement$1 {
  ...
  _sortSelectedByDomOrder() {
    ...
    this.optionSelected.sort((optionA, optionB) => (indexMap.get(optionA) ?? 0) - (indexMap.get(optionB) ?? 0));
