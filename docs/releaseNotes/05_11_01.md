...
- **Keyboard Behavior**
  - **Tab navigation within counter-group** — Individual counters inside a counter-group can now be tabbed through correctly. Previously, only the first counter in the group was reachable via Tab; focus would get stuck and not advance to subsequent counters.
- **UI/UX**
