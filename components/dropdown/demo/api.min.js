...
var styleCss = i$3`.container{display:inline-block;box-sizing:border-box}:host{position:absolute;z-index:1;display:none}:host([fullscreen]){position:fixed;top:0;left:0;width:100dvw;height:100dvh}:host([fullscreen]) .container{width:100%;height:100%}:host([fullscreen])>*{display:flex}:host([data-show]){display:block}:host([common]) .container,:host([rounded]) .container{border-radius:var(--ds-border-radius, 0.375rem)}:host([common]) .container,:host([inset]) .container{padding:var(--ds-size-50, 0.25rem) var(--ds-size-100, 0.5rem)}`;
