...
| `layout`                          |                                   | `string`                                         |             |                                                  |
| `locale`                          | `locale`                          | `string`                                         |             | BCP47 locale tag (e.g. `en-US`, `de-DE`, `ja-JP`). When set, drives the<br />`type="date"` mask pattern (e.g. de-DE → `dd.mm.yyyy`). May also be inherited<br />from a `data-locale` attribute on an ancestor element via `DomHandler.getLocale`. |
