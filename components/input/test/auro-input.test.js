...
describe('auro-input', () => {
  ...
  it('Sets spellcheck and autocapitalize values', async () => {
    ...
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.have.attribute('spellcheck', 'false');
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.have.attribute('autocorrect', 'off');
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.have.attribute('autocapitalize', 'none');
  });
  ...
  it('Does not set spellcheck and autocapitalize values', async () => {
    ...
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.have.attribute('spellcheck', 'true');
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.not.have.attribute('autocorrect');
    await expect(el.shadowRoot.querySelector('#checkSpellCheck')).to.not.have.attribute('autocapitalize');
  });
  ...
  describe('handles i18n', () => {
    it('credit-card translation', async () => {
      ...
      await expect(content).to.not.contain(`Por favor`);
      await expect(eli18nContent).to.contain(`Por favor`);
      await expect(eli18n.shadowRoot.querySelector('#input01')).to.have.attribute('lang', 'es');
    });
  });
