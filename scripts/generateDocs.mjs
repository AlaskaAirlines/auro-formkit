...
/**
 * Replace all instances of @aurodesignsystem, form, Form, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\form(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Form(?!\()/g, nameExtractionData.nameCap);
