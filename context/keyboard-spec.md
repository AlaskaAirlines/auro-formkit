...
**`auro-datepicker` is an exception during this transition period.** Tab does not close the datepicker fullscreen dialog. Tab cycles through focusable elements within the dialog using the browser's native `<dialog>` focus containment. The canonical datepicker keyboard behavior — including planned future bib keyboard navigation — is owned by `components/datepicker/docs/partials/keyboard-behavior.md`.
...
| `auro-datepicker` | `datepickerKeyboardStrategy.js` (dormant placeholder) | No — see `components/datepicker/docs/partials/keyboard-behavior.md` | Escape only (native `<dialog>` cancel event) |
...
- `components/datepicker/src/auro-datepicker.js` — `calendar.focusCloseButton()` on fullscreen dialog open; keyboard behavior intentionally limited during transition (see `components/datepicker/docs/partials/keyboard-behavior.md`)
